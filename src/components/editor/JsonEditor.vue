<template>
  ...
  <div>
    <div style="margin-bottom: 10px">
      <span style="font-size: 15px; text-align: left; font-weight: bold">JSON脚本编辑</span>
    </div>
    <codemirror style="text-align: left" :options="options" v-model="sql" @blur="analyticJson">
    </codemirror>
    <div style="margin-bottom: 10px">
...
import { getParamsToJson } from '@/until/AnalyticSQL'
...
export default {
  ...
  computed: {
    ...
    ...mapState(['inputObj', 'selectScript', 'switchfilter', 'listenerIndex', 'eventListenerObj']),
    tabName: function () {
      return '参数'
    }
  },
    ...
    switch (this.type) {
      case 'elementFormat':
        if (this.getCurrentSelect.tb_window_formatsearch[0].script === null) {
          ...
          this.sql = this.getCurrentSelect.tb_window_formatsearch[0].script.servicevar
      ...
      case 'elementInputFilter':
        if (this.switchfilter !== '') {
          this.sql = this.switchfilter.servicevar
  ...
  methods: {
    ...mapActions(['updateScriptAction', 'updateApiParamViewAction', 'updateFieldsAction', 'updateShowInvokeServiceScriptAction']),
    analyticJson () {
      let obj = {
        type: 'api',
        servicevar: this.sql,
      ...
      let result = this.sql === undefined ? [] : getParamsToJson(this.sql)
      if (result.length > 0) {
    ...
    close () {
      if (this.type === 'invokeService') {
        ...
        this.updateApiParamViewAction(false)
